...
export function LeveragePanel({ data, isLoading }: LeveragePanelProps) {
  if (isLoading) {
    return (
    <div className="w-full rounded-2xl border border-slate-200 bg-white p-8 shadow-sm animate-pulse">
  ...
  return (
    <div className="w-full rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
      <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-900">Agent Leverage Brief</h2>
          <p className="text-xs text-slate-600 font-medium">Mission intel successfully acquired</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
              ...
              {dossier.keyAmenities.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {dossier.keyAmenities.map((amenity) => (
                    <span
                      key={amenity}
                      className="rounded-full border border-cyan-200 bg-cyan-50 px-2.5 py-1 text-xs text-cyan-800"
                    >
                      {amenity}
                    </span>
                  ))}
          ...
          <ul className="space-y-2">
            {data.negotiationPoints.map((point, i) => (
              <li key={i} className="flex items-start gap-3 bg-slate-50 rounded-lg p-3 border border-slate-100">
                <span className="flex-shrink-0 flex items-center justify-center w-6 h-6 rounded-full bg-blue-100 text-blue-700 text-xs font-bold mt-0.5">{i + 1}</span>
            ...
            <div className="space-y-2 text-sm text-slate-600">
              {data.sourceLinks.map((source) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="block rounded-lg border border-slate-200 bg-white shadow-sm px-3 py-2 transition-colors hover:border-blue-300 hover:shadow-md hover:text-blue-700"
                >
                  <span className="font-medium text-blue-600">{source.label}</span>
                  <span className="ml-2 text-xs uppercase tracking-[0.16em] text-slate-400">
                    {source.kind}
                  </span>
                  {source.note && (
                    <span className="block mt-1 text-xs text-slate-500">{source.note}</span>
                  )}
                </a>
              ))}
            </div>
        ...
        {data.contactPhone && (
          <div className="flex items-center gap-3 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl mt-4">
